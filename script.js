...
for(let i=1;i<=100;i++){
    ...
    $("#columns").append(`<div class="column-name" id="col-${i}" >${str}</div>`);
    $("#rows").append(`<div class="row-name" id="row-${i}" >${i}</div>`)
}
...
$(".input-cell").dblclick(function(e){
    $(".input-cell.selected").removeClass("selected top-selected bottom-selected left-selected right-selected gray-background-selected");
...
function unselectCell(ele,e,topCell,bottomCell,leftCell,rightCell){
    if($(ele).attr("contenteditable")=="false"){
        ...
        $(ele).removeClass("selected gray-background-selected");
    }
}
...
function selectCell(ele,e,topCell,bottomCell,leftCell,rightCell){
    if(e.ctrlKey){
        ...
        $(".input-cell.selected").addClass("gray-background-selected");
    }
    else{
        $(".input-cell.selected").removeClass("selected top-selected bottom-selected left-selected right-selected gray-background-selected");
    }
...
let columnArray=$(".column-name");
for(let i=0;i<columnArray.length;i++){
    $(columnArray[i]).click(function(e){
        selectWholeCol(columnArray[i],e);
    });
}
function selectWholeCol(ele,e){
    $(".input-cell.selected").removeClass("selected top-selected bottom-selected left-selected right-selected gray-background-selected");
    let colId=$(ele).attr("id");
    let idArray=colId.split("-");
    let colNumber=parseInt(idArray[1]);
    for(let i=100;i>=1;i--){
        let [rowId,colId]=getRowCol($(`#row-${i}-col-${colNumber}`)[0]);
        let [topCell,bottomCell,leftCell,rightCell]=getTopLeftBottomRightCell(rowId,colId);
        selectCell($(`#row-${i}-col-${colNumber}`)[0],{"ctrlKey":true},topCell,bottomCell,leftCell,rightCell);
    }
}

let rowArray=$(".row-name");
for(let i=0;i<rowArray.length;i++){
    $(rowArray[i]).click(function(e){
        selectWholeRow(rowArray[i],e);
    });
}
function selectWholeRow(ele,e){
    $(".input-cell.selected").removeClass("selected top-selected bottom-selected left-selected right-selected gray-background-selected");
    let rowId=$(ele).attr("id");
    let idArray=rowId.split("-");
    let rowNumber=parseInt(idArray[1]);
    for(let i=100;i>=1;i--){
        let [rowId,colId]=getRowCol($(`#row-${rowNumber}-col-${i}`)[0]);
        let [topCell,bottomCell,leftCell,rightCell]=getTopLeftBottomRightCell(rowId,colId);
        selectCell($(`#row-${rowNumber}-col-${i}`)[0],{"ctrlKey":true},topCell,bottomCell,leftCell,rightCell);
    }
}
...
$(".input-cell").mouseenter(function(e){
    if(e.buttons==1 && startcellSelected==true){
        if(e.pageX < ($(window).width() - 100) && scrollXRStarted) {
    ...
    if(value!=defaultProperties[property]){
        $(".input-cell.selected").each(function(index,data){
            ...
            if(cellData[selectedSheet][rowId-1]==undefined){
                ...
                cellData[selectedSheet][rowId-1][colId-1]={...defaultProperties, "upStream" : [], "downStream": []}; //these 3 dots are used to sparse the object 
                // i.e. we made a copy of defaultProperties Object, bcz we dont want to change the defautProperties object of this particular cell only,
                //we just want to change the properties of the selected cell only thats why we made a copy of defaltProperties
                //object and then did changes in that copied object, so that our defaultProperties object remains as it is.
...
function openFile(){
    ...
    inputFile.change(function(e){
        ...
        reader.onload= ()=> {
            ...
            let sheets=Object.key(cellData)
            lastlyAddedSheet = 1;
            for(let i of sheets){
                if(i.includes("Sheet")) {
            ...
            loadCurrentSheet();
            inputFile.remove();
        }// ye function tab execute hota h jab file read ho jaati h, 
        // aur jo data aata h wo result naam ki key mai store ho jaata h
    });
}
